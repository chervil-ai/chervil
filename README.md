...
- **Your Feeds — the browser reads your feeds for you.** Subscribe to RSS/Atom feeds, podcasts, newsletters, blogs, YouTube channels and subreddits (**Library → Feeds**; no API keys), or **import your whole OPML** from another reader — folders and all. Chervil pulls them quietly in the background; then the **PulseKeeper agent** (or a plain sidebar ask — *"summarize the feeds in the Entertainment folder"*) briefs you on what arrived, grouped by theme and linked, from your own sources with no web search. Feeds group into **folders**, each keeps items as long as you choose (**per-feed retention**), and a 📡 in the toolbar subscribes from any page. Not another feed reader — the one that makes sense of your feeds.
...
- **Sprig on every page** — work with the live site you're viewing: **ask about this page** (💬, selection-aware), **translate it in place** (🌐, any language, one-click restore), have it **read aloud** (🔊), **snip** any region of the screen straight into a question (✂), and **compare your open tabs** in chat ("which tab mentions…").
- **A real multi-tab engine** — every tab runs in its own view: background tabs keep playing (with a clickable 🔊 mute badge), switching is instant with state intact, **tab groups** (named, colored, collapsible, persistent), **tab search** from the address bar ("Switch to tab"), and **truly private tabs** — isolated in-memory sessions whose cookies vanish on close.
- **Switch from Chrome or Edge** — import your **bookmarks, history, saved passwords** (from a browser CSV export, straight into the encrypted vault), and **address autofill** from Chrome, Edge, Brave, or Vivaldi (Settings → Browser → Import, or the first-run **Welcome setup**). Your websites live in **Favorites** (★, with folders + an optional favorites bar); the Chervil pages you compose live in **Saved Pages**, organized by Spaces.
- **Password & card vault** — save logins and payment cards encrypted under a master passphrase (never shown to Sprig, never auto-submitted). Fill them on real sites with 🔑/💳, and **manage** them properly: search, copy to clipboard, and edit in place.
- **Everyday-browser essentials** — the furniture that lets Chervil be your *only* browser: page **zoom** (`Ctrl +/−/0`, remembered per site), **spell-check**, a **downloads** shelf, **printing** (`Ctrl+P`), an inline **PDF viewer**, **send to phone** (📱 QR), **per-site permissions** (camera/mic, location, notifications — asked once and remembered), **favicons**, **reader view** for live sites, **favorites + folders + a favorites bar**, **private tabs** (`Ctrl+Shift+N`), **multiple windows** (`Ctrl+N`), **picture-in-picture**, per-tab **mute**, **address-bar suggestions** from your history, **ad & tracker blocking**, and **clear browsing data**. Chervil can also register as your **default browser** so links from other apps open in it. Prefer a plain web search? A bang (`g!`, `ddg!`, `b!`) does one.
- **Bring your own AI** — pluggable providers: Claude, Grok, Gemini, OpenAI, Azure AI Foundry, and local Ollama (with an optional token for remote servers).
