...
const MAX_LEDGER = 400; // mirrors MAX_LEDGER in the renderer (Trust Ledger cap)
...
const MAX_SAVED_REPLIES = 500; // mirrors MAX_SAVED_REPLIES in the renderer
...
const DT_COLLECTIONS = ['pages', 'trash', 'sites', 'agents', 'schedules', 'collections', 'watchers', 'intents', 'feeds', 'corrections', 'replies'];
...
function mergeStates(sources) {
  ...
  // Saved replies: write-once records (a kept chat answer), so a plain id-keyed
  // union with `at` as recency — and a tombstone can be compared against it
  // honestly, since nothing bumps `at` on its own after the save.
  merged.savedReplies = unionBy(states.map((s) => s.savedReplies), (r) => r && r.id, (r) => r && r.at)
    .filter((r) => survivesTomb(dtMap.replies, r && r.id, r && r.at))
  ...
  // Corrections take the AGENTS rule: hand-authored, additive, and edited in place,
  // so a union keyed by id with newest-edit-wins. Without this they'd inherit the
  // whole-state last-writer-wins default and a correction typed on the laptop would
  // vanish the next time the desktop saved.
  merged.corrections = unionBy(states.map((s) => s.corrections), (c) => c && c.id, (c) => c && (c.updatedAt || c.createdAt))
    .filter((c) => survivesTomb(dtMap.corrections, c && c.id, c && (c.updatedAt || c.createdAt)));
  ...
  // Standing intents follow the watchers rule: recency (lastRun) advances on its own
  // when the intent fires, so a delete tombstone must win unconditionally rather than
  // race an auto-bumped timestamp.
  merged.intents = unionBy(states.map((s) => s.intents), (it) => it && it.id, (it) => it && (it.lastRun || 0))
    .filter((it) => !(it && it.id != null && dtMap.intents.has(String(it.id))));
  // Trust Ledger: append-only, keyed by id, newest write wins. Union across machines
  // and cap so a long-lived multi-device profile can't grow it without bound.
  merged.ledger = unionBy(states.map((s) => s.ledger), (e) => e && e.id, (e) => e && (e.at || 0)).slice(0, MAX_LEDGER);
