...
const DT_COLLECTIONS = ['pages', 'trash', 'sites', 'agents', 'schedules', 'collections', 'watchers'];
...
function mergeStates(sources) {
  ...
  merged.library = {
    history: unionBy(states.map((s) => s.library && s.library.history), (it) => it && it.id, libRecency)
      .filter((it) => survivesTomb(dtMap.pages, it && it.id, libRecency(it)))
      .slice(0, MAX_LIBRARY),
    trash: unionBy(states.map((s) => s.library && s.library.trash), (it) => it && it.id, libRecency)
