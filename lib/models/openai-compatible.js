...
function createOpenAICompatProvider(config = {}) {
  ...
  // returning source URLs as citations. Non-streaming (like geminiChat) — emits the text
  // once so the preview fills, and returns the same { text, citations } shape.
  async function grokResponses({ system, messages, onText = () => {}, groundMode = 'off', onStatus = null }) {
    ...
    const j = await res.json().catch(() => null);
    const { text, citations } = parseResponsesPayload(j);
    if (text) onText(text); // no token streaming on this path — emit once so the preview fills
    return { text, citations: normCitations(citations) };
  }
