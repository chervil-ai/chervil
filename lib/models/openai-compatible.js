...
const PRESETS = {
  grok: {
    ...
    defaultModel: process.env.CHERVIL_GROK_MODEL || process.env.PARSLEE_GROK_MODEL || 'grok-3',
  },
  gemini: {
    label: 'Gemini (Google)',
...
function createOpenAICompatProvider(config = {}) {
  ...
  // Grok grounds via a top-level search_parameters field on this OpenAI-compatible
  // endpoint. Gemini can't ground via the chat/completions shim (its tools array only
  // accepts OpenAI function tools), so Gemini requests are routed to the NATIVE
  // generateContent API (see geminiChat) where the google_search tool works. Azure
  // has no built-in web search.
  ...
  return {
    async run({
      ...
      const dateLine = `\n\nTODAY'S DATE: ${today}. Treat this as the current moment. For "today/latest/recent/this week/last 24 hours", anchor to this date and use live search when available — never present remembered or training-data facts as current.`;
