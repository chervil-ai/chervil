...
function createOpenAICompatProvider(config = {}) {
  ...
  // Grok has provider-native live web search on this OpenAI-compatible endpoint (a
  // top-level search_parameters field). Gemini's Google-Search grounding is NOT
  // available via the chat/completions shim — the tools array only accepts OpenAI
  // function tools, so sending google_search returns HTTP 400 — so Gemini is
  // compose-only here (native-API grounding would be a separate path).
  const canGround = provider === 'grok';
