...
function createOpenAICompatProvider(config = {}) {
  ...
  // Grok grounds via the Agent Tools API on /v1/responses (see grokResponses). Gemini
  // can't ground via the chat/completions shim (its tools array only accepts OpenAI
  // function tools), so Gemini requests are routed to the NATIVE generateContent API
  // (see geminiChat) where the google_search tool works. Azure has no built-in web search.
  ...
  // Grok path: xAI deprecated Live Search (search_parameters → HTTP 410). Grounding now
  // runs through the Agent Tools API on the OpenAI-compatible /v1/responses endpoint:
  // add server-side `web_search` / `x_search` tools and the model searches autonomously,
  // returning source URLs as citations. Streams token deltas (Responses SSE) for a live
  // preview, and treats the final `response.completed` payload as the source of truth for
  // text + citations — so if delta events ever differ, it still degrades to emit-once.
  async function grokResponses({ system, messages, onText = () => {}, groundMode = 'off', onStatus = null }) {
    const baseURL = String(config.baseURL || PRESETS.grok.baseURL).replace(/\/+$/, '');
    ...
    if (groundMode !== 'off') {
      ...
      if (onStatus) onStatus('Sprig is searching the web (Grok)…');
    }
    let res;
    try {
  ...
  async function chat({ system, messages, onText = () => {}, groundMode = 'off', onStatus = null }) {
    ...
    if (provider === 'grok') return grokResponses({ system, messages, onText, groundMode, onStatus });
