...
function createOpenAICompatProvider(config = {}) {
  ...
  async function geminiChat({ system, messages, onText = () => {}, groundMode = 'off', onStatus = null, maxTokens = 0 }) {
  ...
  async function responsesApi({ system, messages, onText = () => {}, groundMode = 'off', onStatus = null, maxTokens = 0 }) {
  ...
  async function chat({ system, messages, onText = () => {}, groundMode = 'off', onStatus = null, maxTokens = 0 }) {
    if (provider === 'gemini') return geminiChat({ system, messages, onText, groundMode, onStatus, maxTokens });
    if (provider === 'grok' || provider === 'openai') return responsesApi({ system, messages, onText, groundMode, onStatus, maxTokens });
