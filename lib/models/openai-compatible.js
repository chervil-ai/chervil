...
// those two Parslee composes from real, current sources (with citations) — not just
// the model's memory:
//   - Grok (xAI Live Search): a top-level `search_parameters` object on the chat-
//     completions request (mode auto/on, sources web/news/x, return_citations) →
//     citations come back as a top-level array.
...
const PRESETS = {
  grok: {
    label: 'Grok (xAI)',
    baseURL: process.env.PARSLEE_GROK_URL || 'https://api.x.ai/v1',
    defaultModel: process.env.PARSLEE_GROK_MODEL || 'grok-3',
  },
  gemini: {
    label: 'Gemini (Google)',
    baseURL: process.env.PARSLEE_GEMINI_URL || 'https://generativelanguage.googleapis.com/v1beta/openai',
    defaultModel: process.env.PARSLEE_GEMINI_MODEL || 'gemini-2.0-flash',
  },
};
...
function createOpenAICompatProvider(config = {}) {
  ...
  // Grok and Gemini have provider-native live web search; Azure does not.
  const canGround = provider === 'grok' || provider === 'gemini';

  // Returns the request-body fields that turn on live web search for this provider,
  // or null if grounding is off / unsupported. `mode`: 'auto' lets the model decide
  // whether to search; 'on' forces it (used for Verify/Deep Dive).
  function groundingFields(mode) {
    if (!canGround || mode === 'off') return null;
    if (provider === 'grok') {
      return {
        search_parameters: {
          mode: mode === 'on' ? 'on' : 'auto',
    ...
    if (provider === 'gemini') {
      // The model decides whether to ground; there's no force flag on this endpoint.
  ...
  if (azure) {
    ...
    const endpoint = String(config.azureEndpoint || process.env.PARSLEE_AZURE_ENDPOINT || '').replace(/\/+$/, '');
    const deployment = config.azureDeployment || process.env.PARSLEE_AZURE_DEPLOYMENT || '';
    const apiVersion = config.azureApiVersion || process.env.PARSLEE_AZURE_API_VERSION || '2024-10-21';
  ...
  return {
    async run({
      ...
      if (refineMode === 'auto' && /<!--\s*parslee:refine\s*-->/i.test(raw)) refine = true;
...
- You MAY include vanilla JavaScript for interactivity. INTERACTIVE APPLETS: Parslee injects a bridge — from a <script>, after guarding with \`if (window.parslee)\`, call \`await window.parslee.ask("...")\` which resolves to \`{ text, sources }\` from Sprig. Use it to build live mini-apps when helpful.
...
function appletPrompt(live) {
  return live
    ? `You are Sprig answering a data request from an interactive applet in a Parslee page. Search the live web when the answer depends on current data, then answer concisely and directly — no preamble, no markdown. If the request asks for JSON, output ONLY valid minified JSON (no prose, no code fences).`
    : `You are Sprig answering a data request from an interactive applet in a Parslee page. Answer concisely and directly from your knowledge — no preamble, no markdown. If the request asks for JSON, output ONLY valid minified JSON (no prose, no code fences). You have no live web access, so do not fabricate real-time values; give a reasonable best-effort answer.`;
}

const REFINE_AUTO_ADDENDUM = `

CONTEXT: The user is viewing a page you composed (its HTML is included with their message). Begin your output with EXACTLY ONE marker comment on its own first line — \`<!-- parslee:refine -->\` if you are revising that page (output the COMPLETE revised HTML), or \`<!-- parslee:new -->\` if this is a new request — then the HTML document.`;
