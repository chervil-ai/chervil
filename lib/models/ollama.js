...
// (default "http://localhost:11434"), CHERVIL_OLLAMA_KEY (optional Bearer token
// for a remote/authenticated Ollama server — local servers need none).
...
function createOllamaProvider(config = {}) {
  ...
  // Optional auth token. Local Ollama needs none, but a remote/reverse-proxied server
  // (or Ollama's own cloud) can require a Bearer token — send it when one is set.
  const apiKey = config.apiKey || process.env.CHERVIL_OLLAMA_KEY || process.env.PARSLEE_OLLAMA_KEY || '';
  const authHeader = apiKey ? { Authorization: `Bearer ${apiKey}` } : {};

  // One streaming chat call against Ollama's /api/chat (NDJSON stream).
  async function chat({ system, messages, onText = () => {}, maxTokens = 0 }) {
    let res;
    try {
      res = await fetch(`${url}/api/chat`, {
        ...
        body: JSON.stringify({
          model,
          stream: true,
          options: { temperature: 0.6, ...(maxTokens > 0 ? { num_predict: maxTokens } : {}) },
  ...
  return {
    async run({
      ...
      if (!verify) {
        ...
        if (recallContext) {
          system += recallMode === 'synthesize' ? DOSSIER_ADDENDUM
            : recallMode === 'augment' ? RECALL_AUGMENT_ADDENDUM
              : RECALL_ADDENDUM;
        }
        if (profile) system += `\n\nABOUT THE USER (tailor to this when helpful): ${String(profile).slice(0, 1500)}`;
        if (corrections) system += `\n\nTHE USER HAS CORRECTED YOU ON THESE POINTS — treat them as authoritative about their own situation, and never repeat a mistake they have already taken the trouble to fix:\n${String(corrections).slice(0, 4000)}\nApply them SILENTLY and ONLY where they apply; never work an irrelevant one into the page, and don't over-generalize a narrow correction. Each is dated — prefer newer evidence where you have it.`;
    ...
    async listModels() {
      try {
        const res = await fetch(`${url}/api/tags`, { headers: authHeader });
        if (!res.ok) return [];
