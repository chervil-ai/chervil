...
// build and test Parslee (tabs, the applet bridge, Spaces, persistence, streaming,
...
// Switch back to PARSLEE_PROVIDER=claude when you need real grounding or page quality.
//
// Config: PARSLEE_OLLAMA_MODEL (default "gemma3:4b"), PARSLEE_OLLAMA_URL
...
function createOllamaProvider(config = {}) {
  const model = config.ollamaModel || process.env.PARSLEE_OLLAMA_MODEL || 'gemma3:4b';
  const url = (config.ollamaUrl || process.env.PARSLEE_OLLAMA_URL || 'http://localhost:11434').replace(/\/+$/, '');
