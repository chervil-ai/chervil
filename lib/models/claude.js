...
function createClaudeProvider(config = {}) {
  ...
  return {
    ...
    async chatReply({ system, messages, maxTokens = 1500, onStatus = () => {} }) {
      ...
      while (guard++ < 5) {
        ...
        if (!announced && msg.content.some((b) => b.type === 'server_tool_use' && b.name === 'web_search')) {
          announced = true;
          onStatus('Sprig is searching the web…');
        }
