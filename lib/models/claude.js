  ...
  return {
    ...
    async run({
      ...
      if (!verify) {
        ...
        if (recallContext) {
          if (recallMode === 'digest') system += DIGEST_ADDENDUM;
          else if (recallMode === 'synthesize') system += DOSSIER_ADDENDUM;
          else system += RECALL_ADDENDUM;
        }
        if (profile) system += profileAddendum(profile);
        if (agent) system += agentAddendum(agent);
        if (pageStyle === 'rich') system += RICH_PAGE_ADDENDUM;
        else if (pageStyle === 'minimal') system += MINIMAL_PAGE_ADDENDUM;
      }
...
function applyAttachments(text, attachments, query) {
  ...
  for (const a of attachments) {
    if (!a) continue;
    if (a.kind === 'text') {
      t += formatTextAttachment(a.name, a.text, query);
