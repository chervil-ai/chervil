...
// PARSLEE_MODEL=claude-opus-4-8 for maximum quality at the cost of latency.
// (The legacy PINGCHAT_* names are still honored as a fallback.)
const DEFAULT_MODEL =
  process.env.PARSLEE_MODEL || process.env.PINGCHAT_MODEL || 'claude-sonnet-4-6';

// Effort controls how much the model deliberates. "low" keeps Parslee snappy;
// bump to PARSLEE_EFFORT=medium/high if you want richer pages and don't mind waiting.
const EFFORT = process.env.PARSLEE_EFFORT || process.env.PINGCHAT_EFFORT || 'low';

// Applet bridge calls (window.parslee.ask) are lightweight live-data lookups, so they
// use a FAST model and minimal research to keep applets responsive. Override with
// PARSLEE_APPLET_MODEL (e.g. set it to claude-sonnet-4-6 for richer answers).
const APPLET_MODEL = process.env.PARSLEE_APPLET_MODEL || 'claude-haiku-4-5-20251001';
...
const SYSTEM_PROMPT = `You are Sprig, the friendly leafy guide of Parslee — the agentic, conversational web browser. Users chat with you by name. Instead of a list of blue links you bring the web alive as a single, beautiful, self-contained web page.

For every user query, do ONE of these two things:

1. COMPOSE A PAGE (the default). Answer the user's request by writing a complete, standalone HTML document. This is the heart of Parslee.
   ...
   - You MAY include vanilla JavaScript for interactivity. INTERACTIVE APPLETS: Parslee injects a live bridge so your page's own JavaScript can call you (Sprig) at runtime. From a <script>, after guarding with \`if (window.parslee)\`, call \`await window.parslee.ask("...")\` — it resolves to \`{ text, sources }\`, a fresh, web-grounded answer. Use this to build genuine mini-apps whenever interactivity or on-demand fresh data makes the page better: trackers, calculators that look real values up, dashboards, comparison tools, "refresh"/"check now" buttons. Wire controls to window.parslee.ask, show a loading state while it runs, and render the returned text into the DOM — ask for JSON in your prompt when you need structured data to parse. For static, evergreen answers, skip the bridge and just compose. No external scripts or frameworks.
   - Your final text response must be ONLY the raw HTML document — no markdown, no code fences, no commentary before or after.

2. OPEN A REAL SITE. If the user clearly wants a specific live website or web app (see the open_website tool), call open_website instead of composing a page.
...
- Decide which it is, then begin your output with EXACTLY ONE of these marker comments, on its own first line:
  <!-- parslee:refine -->   (you are revising the page shown — output the COMPLETE revised HTML document)
  <!-- parslee:new -->      (this is a new, unrelated request — output a fresh HTML document)
...
// System prompt for applet bridge calls (window.parslee.ask). The reply is rendered
// by the page itself, so it must be raw data — not a composed page.
const APPLET_SYSTEM_PROMPT = `You are Sprig, answering a data request coming from an interactive applet embedded in a Parslee page. Be FAST: do at most ONE web search and answer directly from the result snippets — do not read full pages or over-research. Reply with a concise, direct answer to exactly what is asked — no preamble, no sign-off, no markdown. If the request asks for JSON, output ONLY valid minified JSON (no prose, no code fences). Only search when the answer depends on live or recent data; otherwise answer immediately from what you know.`;
...
- You MAY include vanilla JavaScript for interactivity (collapsible sections, a sticky TOC), and the window.parslee.ask bridge (guard with \`if (window.parslee)\`) where a live element helps.
...
function createClaudeProvider(config = {}) {
  ...
  const DEEP_MODEL = process.env.PARSLEE_DEEP_MODEL || model;
  const DEEP_EFFORT = process.env.PARSLEE_DEEP_EFFORT || 'medium';
  ...
  async function deepDive({ query, history = [], onStatus = () => {}, onText = () => {}, spaceContext = null, profile = null }) {
    ...
    const msg2 = await streamWithRetry(
      client,
      {
        ...
        max_tokens: 64000,
        output_config: { effort: DEEP_EFFORT },
        system: DEEP_DIVE_SYSTEM_PROMPT + (profile ? profileAddendum(profile) : ''),
  ...
  return {
    async run({
      ...
      if (deep) return deepDive({ query, history, onStatus, onText, spaceContext, profile });
      ...
      let system = verify ? VERIFY_SYSTEM_PROMPT : SYSTEM_PROMPT;
      if (!verify) {
      ...
      while (guard++ < (verify ? 12 : 8)) {
        ...
        if (refineMode === 'auto' && /<!--\s*parslee:refine\s*-->/i.test(rawText)) {
          refine = true;
        }
      ...
      throw new Error('Parslee could not complete the request (too many tool rounds).');
    },

    // Lightweight, web-grounded Q&A for interactive applets (window.parslee.ask).
...
function extractHtml(text) {
  if (!text) return fallbackPage('Parslee returned an empty page.');
  let t = text.trim();
...
function titleFromQuery(query) {
  ...
  return q.length > 60 ? q.slice(0, 57) + '…' : q || 'Parslee';
}
...
function fallbackPage(body, raw = false) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Parslee</title>
<style>
