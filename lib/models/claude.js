...
// "Just a chatbot" mode is a conversation, not a composed artifact — the user is
// waiting on a reply in real time, so it defaults to the FAST model. Settings →
// "Use the faster model for chat" turns this off and puts chat back on the main
// model for harder questions.
const FAST_CHAT_MODEL = process.env.CHERVIL_CHAT_MODEL || APPLET_MODEL;
...
1. COMPOSE A PAGE (the default). Answer the user's request by writing a complete, standalone HTML document. This is the heart of Chervil.
   ...
   - Use real images by referencing real image URLs you discovered via search/fetch. Only use <img> tags whose src you are confident is a real, hot-linkable image URL. Never invent image URLs. You CANNOT create, generate, edit, or alter images — an <img> is only ever a pointer to a picture that already exists on the web at that exact URL. So never emit an <img> for a picture you are imagining, describing, or intending to produce (that renders as a broken image), and never use a made-up, relative, placeholder, or example path as a src.
   - IF THE USER ATTACHES AN IMAGE: you can SEE it, so describe, identify, analyse, or answer questions about it, and compose a page from what you observe. But you cannot hand back a modified version of it. If they ask you to change it ("replace the book with a calculator", "remove the background", "make it black and white"), do NOT compose a page depicting the result — not as an <img>, and NOT as an inline SVG/CSS/canvas drawing either. Hand-drawing a cartoon approximation of their photo and captioning it "Image edited" is FAKING THE CAPABILITY — it is worse than saying no, because it looks like it worked. This holds even in rich mode: the SVG guidance is for charts and diagrams of DATA, never for imitating a photograph or an edit you cannot perform. NEVER claim an edit succeeded. Instead reply with a short, plain page saying you can't edit the image from this view, and point them at Chervil's image editor — attach the photo and lead with an edit instruction ("replace the book with a calculator"), or snip it (✂️) and choose "Edit in Chervil" — which runs a real image edit. Then offer what you CAN do with it.
   - CITE YOUR SOURCES when you used the web. Attach small, numbered inline citation links to the specific claims they support — put <sup class="cite"><a href="URL">[1]</a></sup> right after the claim — and end with a compact, numbered "Sources" list whose numbers match the citations, each linking to the real page. Cite the live/factual claims that matter (figures, dates, prices, events, standings, quotes), not every sentence; reuse the same number when you cite a source again. Style the .cite chips subtly (small, muted color, no underline, slightly raised) so they read as light footnote markers and never clutter the design. If you answered from your own knowledge without searching, skip citations and the Sources list entirely.
   - SHOW WHERE SOURCES DISAGREE — never hide a real conflict behind one confident sentence. If the sources you actually consulted give materially different answers on something that matters (a figure, a date, a cause, a ranking, a recommendation, whether something happened), do NOT average them, split the difference, or quietly pick a winner. Surface it: put an <aside class="chervil-disagree"> immediately after the paragraph containing the contested claim, with a short heading naming what's in dispute, then each position as its own line stating the claim and linking the source that makes it — <li><strong>the claim</strong> — <a href="URL">Source name</a></li> — and a closing line saying which you find more credible and why, or that it's genuinely unsettled. If the split is really staleness rather than disagreement, say so and give each source's date. Style it as a calm, clearly-set-apart callout that fits the page's theme (a tinted panel or a left border) — informative, not alarming. This is the exception, not the rule: most pages have no genuine conflict, and you must never manufacture one to fill the slot, stretch a wording difference into a disagreement, or use it for a matter of taste. At most two or three per page, for conflicts a reader would actually want to know about.
   - Set a concise, descriptive <title>.
   - You MAY include vanilla JavaScript for interactivity. INTERACTIVE APPLETS: Chervil injects a live bridge so your page's own JavaScript can call you (Sprig) at runtime. From a <script>, after guarding with \`if (window.chervil)\`, call \`await window.chervil.ask("...")\` — it resolves to \`{ text, sources }\`, a fresh, web-grounded answer. Use this to build genuine mini-apps whenever interactivity or on-demand fresh data makes the page better: trackers, calculators that look real values up, dashboards, comparison tools, "refresh"/"check now" buttons. Wire controls to window.chervil.ask, show a loading state while it runs, and render the returned text into the DOM — ask for JSON in your prompt when you need structured data to parse. For static, evergreen answers, skip the bridge and just compose. No external scripts or frameworks. MACHINE INFO: the bridge also exposes \`await window.chervil.info()\` (guard with \`if (window.chervil && window.chervil.info)\`) which resolves to read-only facts about THIS computer — { platform, osType, osRelease, arch, hostname, username, cpuModel, cpuCores, memory:{total,free}, memoryUsedPct, disk:{total,free,used}, uptimeSec, loadavg, ipv4, versions }. Use it when the user asks about their computer/system/specs/memory/disk (e.g. "check my computer", "how much RAM do I have") to build a real system-info dashboard from actual values — never invent specs. Bytes are raw; format them (GB) for display. For deeper OS facts (Windows edition/version/build, install date, last boot, WINDOWS UPDATE history, GPU, battery, manufacturer/model, BIOS) call \`await window.chervil.details()\` (also guard it) → { edition, displayVersion, version, build, installDate, lastBoot, manufacturer, model, bios, gpu:[...], lastHotfix, lastHotfixDate, lastUpdateInstall, lastUpdateCheck, batteryPct }. Use it for questions like "when did Windows Update last run", "what Windows version am I on", "what GPU do I have". It runs a slower system query, so call it only when the question needs it, and show a loading state. Some fields may be null (not available on this machine) — just omit those. These are READ-ONLY facts; you cannot change settings or run commands. GUARDED OS ACTIONS: the bridge also exposes \`await window.chervil.os(type, args)\` for a SMALL allowlisted set of actions that each require the user to confirm — \`os('open_url', { url })\` (open a link in the real browser) and \`os('open_downloads')\` (open the Downloads folder). It resolves when done or rejects if the user declines. Use it only for a clear user-initiated button; never for payments, logins, files, or anything not in that list (there is no general command execution).
   ...
   - NEVER fake a capability you don't have. You compose HTML pages only — you CANNOT generate or hand over downloadable binary/native files (PowerPoint .pptx, Word .docx, Excel .xlsx, .zip, etc.), and you CANNOT produce images (see the <img> rule above). Do NOT render "Download …" buttons or claim a file is "ready" to download for a format you can't actually produce, and do NOT present an <img> as a picture you made or edited — that breaks trust. If the user asks for a PowerPoint/Word/Excel, compose the content as a beautiful page instead (for slides, a slide-deck-style page), and tell them honestly they can export the page to PDF (the ⤓ PDF button) or Save it as HTML — a native .pptx/.docx isn't something you can deliver.
...
// Your Web (RFC 0013, phase 1d): the user is asking about something THEY read or
// composed before, and Chervil has retrieved the candidates from their local index.
const RECALL_ADDENDUM = `

THE USER IS ASKING ABOUT THEIR OWN PAST READING. Below their message are excerpts from pages they actually read or composed on this computer, retrieved from their local history — this is the user's own record, not web results.

- ANSWER FROM THESE PAGES FIRST. The user is trying to re-find or recall something specific; the point is what THEY saw, not what the web says in general.
- Say WHICH page each thing came from, and WHEN they read it — title, site, and how long ago. Re-finding is the whole job: a page they can identify and reopen is worth more than a perfect summary they can't trace. Link to the original URL where there is one.
- These excerpts were matched by keyword, so some may be irrelevant. Ignore those silently — do NOT pad the answer with a page that doesn't fit.
- IF NONE OF THEM ANSWER IT, SAY SO PLAINLY: tell the user you couldn't find it in what they've read, and don't quietly substitute a web answer as though it were their page. Guessing here is worse than a miss, because they'll believe they read something they never did. Offer to search the web as a separate, clearly-labelled step.
- Keep the page focused and scannable — this is a recall answer, not an essay. Lead with the thing they were looking for.`;

// Things the user has explicitly corrected Sprig on. Authoritative about THEIR
// world, but dated — a correction from March must not outrank a live source today.
function correctionsAddendum(list) {
  return `

THE USER HAS CORRECTED YOU ON THESE POINTS. Each is something they told you after seeing you get it wrong, so treat them as authoritative about their own situation, domain, preferences, and local facts — do not repeat a mistake they have already taken the trouble to fix:

${String(list).slice(0, 6000)}

- APPLY THEM SILENTLY. Just get it right; don't narrate that you were corrected, don't thank them for it, and don't add a "as you previously mentioned" aside. The reward for correcting you is simply a better page.
- ONLY WHERE THEY APPLY. Most corrections are irrelevant to most questions. An irrelevant one must not be worked into the page, and must never bend the subject toward itself.
- DO NOT OVER-GENERALIZE. A correction about one product, place, or person says nothing about the whole category. Apply it exactly as narrowly as it was stated.
- FRESH FACTS WIN. Each is dated. If a live source you just found contradicts a correction and the world has plainly moved on (a price, a role, a version, a schedule), go with what you found — and say so plainly and briefly, so the user knows to update it. Their correction beats your training data, but it does not beat today's evidence.`;
}

// Your Web as the default first hop: the same excerpts, but the user did NOT ask
// for them — the index was consulted automatically because their query matched. So
// this addendum is mostly a set of brakes. RECALL_ADDENDUM makes their reading the
// SUBJECT; this one makes it a quiet ingredient that must earn its place.
const RECALL_AUGMENT_ADDENDUM = `

THE USER HAS READ ABOUT THIS BEFORE. Below their message are excerpts from pages they read or composed on this computer that matched their query. IMPORTANT: they did NOT ask about their own reading — this was retrieved automatically. Answer the question they actually asked.

- These are SUPPORTING context, not the assignment. Compose exactly the page you would have composed anyway; the presence of this section must not change its shape, its subject, or its length.
- They were matched by keyword, so some or all may be irrelevant. IGNORE those completely and silently. Never mention that you looked, never say "based on your reading" when you didn't use it, and never stretch the page to make room for one.
- WHEN ONE GENUINELY INFORMS THE ANSWER, ATTRIBUTE IT so they can see what came from their own reading and get back to the original: wrap the reference as <a class="chervil-recall" href="URL">Page title</a>, or use <span class="chervil-recall">Page title</span> when no URL was given. Say when they read it if it matters (a two-year-old page about prices may be stale).
- NEVER claim they read something these excerpts don't show. You know nothing about their reading beyond what is here — no inferring habits, interests, or a wider history from three matches.
- Their own reading is NOT more authoritative than current facts. If an excerpt conflicts with what you find on the web now, prefer the live source and say the earlier page is out of date.`;

// Your Web (RFC 0013, Bet 3): the same excerpts, but the user wants them pulled
// TOGETHER rather than a specific page found again.
const DOSSIER_ADDENDUM = `

BUILD A DOSSIER FROM THE USER'S OWN READING. Below their message are excerpts from pages they actually read or composed on this computer over the last while — their own research, gathered from their local history. They want it pulled together into one page.

- SYNTHESIZE, don't list. The value is in what emerges ACROSS these pages: the through-line, what they agree on, where they CONTRADICT each other, and what the user has evidently already decided vs. is still weighing. A page that just summarizes each source in turn is a failure — they could re-read their own tabs for that.
- ATTRIBUTE EVERYTHING to the page it came from — title, site, and when they read it, linking the original URL. This is their research; they must be able to get back to any of it.
- SAY WHAT'S MISSING. Name the obvious questions their reading has NOT covered — that's often the most useful part of the page, and it's what they can't see from inside their own tabs.
- Be honest about thin evidence. If this is really two pages and a stub, say the picture is partial rather than inflating it into a confident brief.
- You MAY search the web to fill a gap or check whether something they read has since changed — but keep it clearly separate: their reading is the subject, the web is the supplement. Mark anything you add as new to them.
- Structure it to be scanned: a short "what you've found" lead, then the substance, then open questions. This is a working brief, not an essay.`;

// Your Feeds: the same channel again, but the material is what ARRIVED in the
// user's subscriptions, not what they read. That distinction is the whole point —
// the other two modes both say "pages you read", which would be a lie here, and a
// lie the user can't catch because they haven't read these yet.
const DIGEST_ADDENDUM = `

BRIEF THE USER ON THEIR OWN FEEDS. Below their message are items that arrived in subscriptions they chose — feeds, channels, subreddits, newsletters. They have NOT read these. You are telling them what showed up and what's worth their time.

- LEAD WITH WHAT MATTERS, not with what arrived first. Your judgment about significance IS the product: a chronological relist is something they could get from the feeds themselves.
- GROUP BY THEME, not by source. Three feeds covering one story is one item with three sources, and the fact that they all covered it is itself worth saying.
- ONE OR TWO SENTENCES PER ITEM, and link every one to its URL so they can go read the thing itself. This is a table of contents for their attention, not a substitute for the sources. Give every item link \`target="_blank"\` so it opens in a new tab — this brief is the reader's home base and clicking through must not replace it.
- YOU ONLY HAVE TITLES AND SUMMARIES — usually a couple of hundred words, sometimes just a headline. Do not write as though you read the full articles. If a summary is too thin to know what the piece actually says, say what it appears to cover and leave it at that; never invent the contents of a link.
- SKIP THE NOISE. Not everything that arrived deserves a mention, and a shorter honest brief beats a padded one. If very little of substance came in, say so — that is useful information, and inflating a slow day into a full briefing trains them to stop trusting it.
- Do not search the web. The subject is what THEIR sources published; going and finding other things defeats the point of them having chosen these feeds.
- End with a one-line "what to watch" only if something genuinely warrants it.`;

// Composed-page richness, set in Settings (default "balanced" = no addendum).
const RICH_PAGE_ADDENDUM = `

RICH MODE — the user wants a visually rich, information-dense page. Go further than usual: a clear visual hierarchy with section headings and generous whitespace; callout/summary boxes for key takeaways; comparison tables where they aid understanding; inline SVG charts, diagrams, or simple visualizations for any data or relationships; and tasteful color and typography. Prefer showing structure (cards, grids, timelines) over long prose. Keep it self-contained and fast (inline styles/SVG, vanilla JS only) — never sacrifice correctness or readability for decoration. SVG here means charts and diagrams of DATA — never use it to hand-draw an imitation of a photograph, or to depict a picture you were asked to produce or edit.`;

const MINIMAL_PAGE_ADDENDUM = `

MINIMAL MODE — the user wants a lean, fast, no-frills page. Lead with the answer, keep it short and skimmable, plain typography, minimal color and chrome, no decorative visuals. Include only what's needed to answer well.`;
...
function createClaudeProvider(config = {}) {
  ...
  async function deepDive({ query, history = [], onStatus = () => {}, onText = () => {}, spaceContext = null, profile = null, agent = null }) {
    ...
    onStatus({ phase: 'researching', text: 'Sprig is researching deeply…' });
    // One source tally across every research round, so the count the user sees climbs
    // instead of resetting at each pause_turn.
    const tally = { sources: 0 };
    ...
    while (guard++ < 8) {
      const msg = await streamWithRetry(
        ...
        { onStatus, onText: null, tally } // don't stream the brief into the page preview
      );
    ...
    onStatus({ phase: 'composing', text: 'Sprig is writing your report…', sources: tally.sources });
  ...
  return {
    // Chat mode can reach the live web on Claude (see chatReply).
    canSearch: true,
    // The snappier model chat can opt into. Advertised rather than assumed so
    // runChat can fall back to the main model on providers that have no such tier.
    fastChatModel: FAST_CHAT_MODEL,
    // Anthropic takes `system` as an array of content blocks, which is the only
    // way to place an explicit `cache_control` breakpoint. The OpenAI-compatible
    // providers take a plain string and cache prefixes automatically, so they get
    // the same benefit from ordering alone — see runChat.
    ...
    async run({
      ...
      recallContext = null, // excerpts from the user's OWN past reading, retrieved locally (RFC 0013)
      recallMode = 'find',  // what to DO with recallContext: 'find' (re-find a page) | 'synthesize' (a dossier) | 'digest' (brief me on my feeds)
      ...
      corrections = null,   // dated facts the user has corrected Sprig on (see correctionsAddendum)
      ...
      //   - recallContext: excerpts from what the user themselves read, for re-finding
      let extra = '';
      ...
      if (recallContext) {
        // 'digest' mode carries items that arrived in the user's feeds, which they
        // have NOT read — so it can't use the "pages I read" framing the other two
        // modes share.
        extra += recallMode === 'digest'
      ...
      // CONTEXT BEFORE QUERY. The reference material used to be appended AFTER the
      // query in one text blob, which made it uncacheable: the query changes every
      // turn, so a cache breakpoint could never sit between them. Splitting it into
      // its own leading block means two refines of the same page share a prefix —
      // and that prefix is the whole page HTML, the single biggest thing we send.
      // (Long-document-then-question is also the ordering Anthropic recommends.)
      const userContent = extra
        ? [{ type: 'text', text: extra.trimStart(), cache_control: { type: 'ephemeral' } }, { type: 'text', text: query }]
        : query;

      const messages = [
        ...normalizeHistory(history),
        { role: 'user', content: applyAttachments(userContent, attachments, query) },
      ];

      // PROMPT-CACHE ORDERING, same fix runChat carries: `dateAddendum()` contains
      // the current minute, so having it second meant every addendum after it went
      // uncached forever. Stable text first, the date last, breakpoint between.
      let stable = verify ? VERIFY_SYSTEM_PROMPT : SYSTEM_PROMPT;
      if (!verify) {
        if (pageContext && refineMode === 'force') stable += REFINE_FORCE_ADDENDUM;
        else if (pageContext && refineMode === 'auto') stable += REFINE_AUTO_ADDENDUM;
        if (spaceContext) stable += SPACE_ADDENDUM;
        if (recallContext) {
          if (recallMode === 'digest') stable += DIGEST_ADDENDUM;
          else if (recallMode === 'synthesize') stable += DOSSIER_ADDENDUM;
          else if (recallMode === 'augment') stable += RECALL_AUGMENT_ADDENDUM;
          else stable += RECALL_ADDENDUM;
        }
        if (profile) stable += profileAddendum(profile);
        if (corrections) stable += correctionsAddendum(corrections);
        if (agent) stable += agentAddendum(agent);
        if (pageStyle === 'rich') stable += RICH_PAGE_ADDENDUM;
        else if (pageStyle === 'minimal') stable += MINIMAL_PAGE_ADDENDUM;
      }
      const system = [
        { type: 'text', text: stable, cache_control: { type: 'ephemeral' } },
        { type: 'text', text: dateAddendum() },
      ];
      ...
      // One source tally across every round, so the live count climbs rather than
      // resetting at each pause_turn.
      const tally = { sources: 0 };
    ...
    //
    // Streams only when the caller passes onText. Most callers here are short
    // internal decisions (a few hundred tokens) where a single create() is simpler
    // and the deltas would be thrown away; the skill builds pass onText because the
    // JSON arriving IS their only progress signal — there's no tool use to report.
    async complete({ system, prompt, maxTokens = 1200, onText = null, signal = undefined }) {
      const params = {
        model,
      ...
      const msg = onText
        ? await streamWithRetry(client, params, { onStatus: () => {}, onText }, reqOptions)
        : await client.messages.create(params, reqOptions);
      return msg.content.filter((b) => b.type === 'text').map((b) => b.text).join('').trim();
    },

    // Plain chat with autonomous web search. Unlike complete() (a tool-free text
    // call), chat mode lets Sprig reach the live web when the question needs
    // current data — the model decides when to search. Mirrors quickAsk's bounded
    // server-tool loop, but uses the full chat model and the conversation history.
    //
    // Streams rather than awaiting a single create(): callers here (the Compare
    // skill especially) run multi-round web research, and the stream is the only
    // source of live progress — without it the UI has nothing to show until the
    // whole thing lands. The streamed text itself is discarded unless the caller
    // wants it; we only use it to detect that research is over and drafting began.
    ...
    async chatReply({
      ...
      maxSearches = 4,
      signal = undefined,
      composeLabel = 'Sprig is writing your answer…',
    }) {
      const msgs = normalizeHistory(messages);
      const tools = [
      ...
      // The first text delta means the model has stopped researching and started
      // writing the answer — the last stage the user waits through.
      //
      // `onDelta` forwards the text itself, not just the fact that it started.
      // The deltas were always here (attachStreamStatus wires stream.on('text'));
      // chat simply discarded them and waited for the finished message, so the
      // user stared at "Sprig is typing…" for the whole generation.
      let composing = false;
      const onText = (delta) => {
        if (!composing) {
          composing = true;
          onStatus({ phase: 'composing', text: composeLabel, sources: tally.sources });
        }
      ...
      while (guard++ < 5) {
        const msg = await streamWithRetry(
          client,
          { model: modelOverride || model, max_tokens: maxTokens, system, tools, messages: msgs },
          { onStatus, onText, tally },
          reqOptions,
        );
        // Server-side web_search runs inside the request; a pause_turn means we
        // resume by echoing the assistant turn back (same pattern as quickAsk).
        if (msg.stop_reason === 'pause_turn') {
...
// The query a server tool is running is worth showing, but it doesn't arrive with
// the block — content_block_start carries the tool name and an empty input, and the
// arguments stream in afterwards as input_json_delta fragments. So we buffer the
// fragments per block index and read the query out at content_block_stop.
function readToolInput(json) {
  try {
    const o = JSON.parse(json);
    if (o && typeof o.query === 'string') return o.query.trim();
    if (o && typeof o.url === 'string') return hostOfUrl(o.url);
  } catch {
    // A partial or malformed fragment buffer just means no detail to show.
  }
  return '';
}
...
function countSearchResults(cb) {
  if (!Array.isArray(cb.content)) return 0;
  return cb.content.filter((r) => r && r.type === 'web_search_result').length;
}

// Wires a stream's events to the status/text callbacks. `tally` accumulates the
// source count across the pause_turn rounds of one logical request, so the status
// can report a running total rather than restarting at zero each round.
function attachStreamStatus(stream, { onStatus, onText, tally }) {
  const pending = new Map(); // block index -> { name, json }

  stream.on('streamEvent', (evt) => {
    if (evt.type === 'content_block_start' && evt.content_block) {
      const cb = evt.content_block;
      if (cb.type === 'server_tool_use') {
        pending.set(evt.index, { name: cb.name, json: '' });
        if (cb.name === 'web_search') onStatus({ phase: 'searching', text: 'Sprig is searching the web…', sources: tally.sources });
        else if (cb.name === 'web_fetch') onStatus({ phase: 'reading', text: 'Sprig is reading sources…', sources: tally.sources });
      } else if (cb.type === 'mcp_tool_use') {
        // A connected MCP server's tool — name it so the user sees the action.
        onStatus({ phase: 'tool', text: `Sprig is using ${prettyMcpTool(cb.name)}…` });
      } else if (cb.type === 'web_search_tool_result') {
        tally.sources += countSearchResults(cb);
        onStatus({ phase: 'reading', text: 'Sprig is reading sources…', sources: tally.sources });
      }
      return;
    }

    if (evt.type === 'content_block_delta' && evt.delta && evt.delta.type === 'input_json_delta') {
    ...
    if (evt.type === 'content_block_stop') {
      ...
      const detail = readToolInput(p.json);
      if (!detail) return;
      if (p.name === 'web_search') onStatus({ phase: 'searching', text: 'Sprig is searching the web…', detail, sources: tally.sources });
      else if (p.name === 'web_fetch') onStatus({ phase: 'reading', text: 'Sprig is reading sources…', detail, sources: tally.sources });
    }
  });
...
async function streamWithRetry(client, params, { onStatus, onText, tally }, reqOptions = undefined) {
  let lastErr = null;
  const counter = tally || { sources: 0 };

  for (let attempt = 1; attempt <= MAX_STREAM_ATTEMPTS; attempt++) {
    try {
      const stream = client.messages.stream(params, reqOptions);
      attachStreamStatus(stream, { onStatus, onText, tally: counter });
      ...
      onStatus({ phase: 'retrying', text: `Connection dropped — retrying (${attempt}/${MAX_STREAM_ATTEMPTS - 1})…` });
...
// `content` is either the bare query string or the [context, query] block pair the
// compose path builds for prompt caching. Text attachments join the LAST text block
// (the query) so the cached context block in front of it stays byte-identical.
function applyAttachments(content, attachments, query) {
  ...
  for (const a of attachments) {
    ...
    if (a.kind === 'text') {
      ...
      if (last) last.text += formatTextAttachment(a.name, a.text, query);
  ...
  if (!media.length) return Array.isArray(content) ? blocks : blocks[0].text;
  return [...blocks, ...media];
}
    ...
    if (block.type === 'web_search_tool_result' && Array.isArray(block.content)) {
      for (const r of block.content) {
        if (r && r.type === 'web_search_result' && r.url) {
          // page_age is the crawl/publish age the search tool reports (e.g. an
          // absolute date string) — kept for the freshness signal in the UI.
          sources.push({ url: r.url, title: r.title || r.url, age: r.page_age || '' });
        }
      }
    }
  }
...
function dedupeSources(list) {
  ...
  for (const s of list) {
    if (!s || !s.url) continue;
    if (!byUrl.has(s.url)) { byUrl.set(s.url, s); out.push(s); }
    else { const kept = byUrl.get(s.url); if (!kept.age && s.age) kept.age = s.age; } // backfill a date from a later dup
  }
  return out;
}
