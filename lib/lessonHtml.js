...
// Baked widgets (published/exported) run in a sandboxed data: iframe. Unlike srcdoc,
// a data: URL document does NOT inherit the reader's strict CSP — it gets its own
// (empty) policy — so the widget's own inline script runs while the main document
// stays locked down. The widget reports its height so the frame can size to fit.
// Reports the widget's height so the host frame can size to fit. Dynamic resizes
// (interactive widgets that grow/shrink) are caught by ResizeObserver — NOT a
// perpetual interval, which would burn CPU/battery forever in every embedded
// widget. The capped interval is only a fallback for browsers without ResizeObserver.
const APPLET_HEIGHT_REPORTER = `<script>(function(){function h(){try{parent.postMessage({__appletH:Math.max(document.body?document.body.scrollHeight:0,document.documentElement.scrollHeight)},'*');}catch(e){}}window.addEventListener('load',h);setTimeout(h,250);setTimeout(h,1000);if(window.ResizeObserver){try{new ResizeObserver(h).observe(document.documentElement);return;}catch(e){}}var n=0,t=setInterval(function(){h();if(++n>=8)clearInterval(t);},1000);})();</script>`;
function appletFrameSrc(html) {
  const doc = String(html || '') + APPLET_HEIGHT_REPORTER;
  return 'data:text/html;charset=utf-8;base64,' + Buffer.from(doc, 'utf8').toString('base64');
}

// A data: iframe inherits the embedder's CSP (data: is a "local scheme"), so the
// widget's own inline <script> is checked against the reader's script-src. Collect
// the SHA-256 of each inline script block so they can be added to the policy — the
// only way to run the (intended, isolated) widget code while still blocking stray
// inline scripts in the main document. Inline event-handler attributes and eval are
// NOT covered by hashes, so the widget prompt forbids them.
function inlineScriptHashes(html) {
  const out = [];
  const re = /<script\b([^>]*)>([\s\S]*?)<\/script>/gi;
  let m;
  while ((m = re.exec(String(html || '')))) {
    if (/\bsrc\s*=/i.test(m[1])) continue; // external script — not inline, no hash
...
function appletInner(c) {
  // Baked interactive WIDGET (published/exported): composed at publish time with the
  // author's local key, embedded as a sandboxed data: iframe — fully self-contained,
  // no live bridge. Falls back to a baked text answer, then to a live "Try it" button
  // (in-app, wired to window.chervil.applet).
  if (c.appletHtml) {
    return `<div class="applet"><p class="desc">${esc(c.prompt)}</p><iframe class="appletframe" sandbox="allow-scripts" referrerpolicy="no-referrer" src="${appletFrameSrc(c.appletHtml)}"></iframe></div>`;
  }
  if (c.answer) {
    return `<div class="applet"><p class="desc">${esc(c.prompt)}</p><div class="out snapshot">${esc(c.answer)}</div></div>`;
  }
...
const INTERACTIVE_JS = `(function(){
  ...
  // Self-contained widgets report their height so the inline frame can size to fit.
  window.addEventListener('message',function(e){ var d=e.data; if(!d||typeof d.__appletH!=='number') return;
    var fr=document.querySelectorAll('iframe.appletframe'); for(var i=0;i<fr.length;i++){ if(fr[i].contentWindow===e.source){ fr[i].style.height=Math.min(1400,Math.max(180,d.__appletH+8))+'px'; } } });
  var HREP='<scr'+'ipt>(function(){function h(){try{parent.postMessage({__appletH:Math.max(document.body?document.body.scrollHeight:0,document.documentElement.scrollHeight)},"*");}catch(e){}}window.addEventListener("load",h);setTimeout(h,250);setTimeout(h,1000);if(window.ResizeObserver){try{new ResizeObserver(h).observe(document.documentElement);}catch(e){}}})();</scr'+'ipt>';
  document.querySelectorAll('.applet').forEach(function(a){
    var btn=a.querySelector('.run'), out=a.querySelector('.out');
    if(!btn) return;
    function fail(msg){
      out.hidden=false; out.classList.remove('framed'); out.classList.add('err'); out.textContent='';
      var s=document.createElement('span'); s.textContent='Couldn’t build this — '+msg+'. ';
      var r=document.createElement('button'); r.className='retry'; r.textContent='Retry';
      r.addEventListener('click',function(){ run(true); });
      out.appendChild(s); out.appendChild(r);
      btn.disabled=false; btn.textContent=a.dataset.built?'↻ Regenerate':(btn.getAttribute('data-label')||'▶ Try it with Sprig');
    }
    function run(force){
      if(!(window.chervil&&window.chervil.applet)){ out.hidden=false; out.classList.add('err'); out.textContent='Interactive features need the Chervil app.'; return; }
      if(!btn.getAttribute('data-label')) btn.setAttribute('data-label', btn.textContent);
      btn.disabled=true; btn.textContent=force?'Rebuilding…':'Building…'; out.hidden=false; out.classList.remove('err'); out.classList.remove('framed'); out.textContent='';
      window.chervil.applet(a.getAttribute('data-prompt'), force).then(function(r){ var html=r&&r.html;
        if(!html){ return fail('no widget returned'); }
        var f=document.createElement('iframe'); f.className='appletframe'; f.setAttribute('sandbox','allow-scripts'); f.setAttribute('referrerpolicy','no-referrer'); f.srcdoc=String(html)+HREP;
        out.classList.add('framed'); out.textContent=''; out.appendChild(f);
        a.dataset.built='1'; btn.disabled=false; btn.textContent='↻ Regenerate'; done(a);
      }).catch(function(e){ fail((e&&e.message)||e); });
    }
    btn.addEventListener('click', function(){ run(a.dataset.built==='1'); });
  });
})();`;
...
const COMPONENT_CSS = `
  ...
  *{box-sizing:border-box}
  ...
  .applet .retry{background:var(--green);color:var(--bg);border:0;border-radius:8px;padding:6px 12px;font:inherit;font-weight:600;cursor:pointer;vertical-align:middle}
  ...
  .applet .out.framed{padding:0;background:none;border:0}
  .applet .appletframe{display:block;width:100%;height:300px;border:1px solid var(--line);border-radius:10px;background:#0b0d12}
`;
...
function readerDoc(lesson, total) {
  ...
  var lastIdx=-1, started=false, doneSent=false;
  // Baked applet widgets (sandboxed data: iframes) report their height so we can size the frame.
  window.addEventListener('message',function(e){var d=e.data;if(!d||typeof d.__appletH!=='number')return;var fr=document.querySelectorAll('iframe.appletframe');for(var i=0;i<fr.length;i++){if(fr[i].contentWindow===e.source){fr[i].style.height=Math.min(1400,Math.max(180,d.__appletH+8))+'px';}}});
  function emit(type,idx){try{parent.postMessage({__chervil_reader:1,type:type,cardIdx:(typeof idx==='number'?idx:null),total:total},'*');}catch(e){}}
  function update(){
    var s=Math.round(deck.scrollTop/deck.clientHeight);
    var i=Math.min(total,s+1);if(pbar)pbar.style.width=Math.round(i/total*100)+'%';
    var idx=Math.min(total-1,Math.max(0,s));
    if(idx!==lastIdx){lastIdx=idx;if(!started){started=true;emit('start',null);}emit('card',idx);if(idx>=total-1&&!doneSent){doneSent=true;emit('complete',null);}}
  }
...
${INTERACTIVE_JS}`;
  ...
  // Baked widgets run in sandboxed data: iframes (frame-src data:). Those inherit
  // this CSP, so each widget's inline <script> must be hash-allowlisted here for it
  // to run, while any non-allowlisted inline script (e.g. model-injected into a
  // concept card) stays blocked. The height-reporter we append to each widget is
  // hashed too.
  const widgetHashes = new Set();   // inline <script> blocks in baked widgets
  for (const m of (lesson.modules || [])) for (const c of (m.cards || [])) {
    if (c.kind === 'applet' && c.appletHtml) {
      inlineScriptHashes(String(c.appletHtml) + APPLET_HEIGHT_REPORTER).forEach((h) => widgetHashes.add(h));
    }
  }
  const scriptSrc = ["'sha256-" + scriptHash + "'", ...widgetHashes].join(' ');
  const frameSrc = widgetHashes.size ? 'frame-src data:; ' : '';
  const csp = `default-src 'none'; img-src https: data:; style-src 'unsafe-inline'; script-src ${scriptSrc}; ${frameSrc}base-uri 'none'; form-action 'none'`;
...
<style>${COMPONENT_CSS}
  ...
  .slide{min-height:100dvh;scroll-snap-align:start;display:flex;flex-direction:column;justify-content:center;padding:48px 20px;border-bottom:1px solid rgba(255,255,255,.04)}
  .slide-in{max-width:680px;margin:0 auto;width:100%}
  @media(min-width:1000px){ .slide{padding:64px 48px} .slide-in{max-width:920px} h3{font-size:24px} .concept{font-size:18px} }
