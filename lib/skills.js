...
const { runBuildLesson, runBuildQuiz, runBuildCompare } = require('./agent');
const { lessonToHtml } = require('./lessonHtml');
const { cardCount } = require('./lesson');
const { verifyLessonMedia } = require('./lessonMedia');
const { quizToHtml } = require('./quizHtml');
const { questionCount } = require('./quiz');
const { compareToHtml } = require('./compareHtml');
const { dimensionCount } = require('./compare');
 ...
 * @property {(opts:{input:string, level?:string, goals?:string, config?:object, onStatus?:(s:object)=>void, signal?:AbortSignal})=>Promise<object>} build
 *           Produce a validated artifact from the user's request. onStatus reports
 *           live progress to the UI; signal cancels an in-flight build (Stop).
 ...
 * @property {((artifact:object, config:object)=>Promise<void>)} [enrich]
 *           Optional post-build async step (e.g. Learn verifies media). Mutates in place.
 */
...
const learnSkill = {
  ...
  command: '/learn',
  build({ input, level = 'beginner', goals = '', config = {}, onStatus, signal }) {
    return runBuildLesson({ topic: input, level, goals, config, onStatus, signal });
  },
  enrich(lesson) {
    return verifyLessonMedia(lesson); // oEmbed-verify YouTube media
  },
  toHtml(lesson, opts = {}) {
...
const quizSkill = {
  ...
  name: 'Quiz',
  emoji: '❓',
  description: 'Build a graded multiple-choice quiz on any topic.',
  command: '/quiz',
  build({ input, level = 'beginner', goals = '', config = {}, onStatus, signal }) {
    return runBuildQuiz({ topic: input, level, goals, config, onStatus, signal });
  },
  toHtml(quiz) {
    return quizToHtml(quiz);
  },
  summarize(quiz) {
    return { title: quiz.title || 'Quiz', summary: quiz.summary || '', count: questionCount(quiz) };
  },
};
...
const compareSkill = {
  ...
  name: 'Compare',
  emoji: '⚖️',
  description: 'Build a sourced, side-by-side comparison to help you decide.',
  command: '/compare',
  build({ input, config = {}, onStatus, signal }) {
    return runBuildCompare({ input, config, onStatus, signal });
  },
  toHtml(cmp) {
    return compareToHtml(cmp);
  },
  summarize(cmp) {
    return {
      title: cmp.title || 'Comparison',
      summary: cmp.summary || '',
      count: dimensionCount(cmp),
    };
  },
};

const SKILLS = [learnSkill, quizSkill, compareSkill];
