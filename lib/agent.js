...
function chatSystem(canSearch) {
  ...
  return `${base}${web} Keep a warm, helpful tone. Plain text only.`;
}
...
async function runChat({ query, history = [], profile = null, pageContext = null, pageMeta = null, tabsContext = null, config = {}, onStatus = () => {}, onDelta = null, signal = undefined, chatModel = '' }) {
  ...
  if (useReply) {
    ...
    return { text: String(text || '').trim(), sources: Array.isArray(sources) ? sources : [] };
  }
...
- Prefer primary/authoritative sources; if sources disagree on a value, use the most credible and note it. Never invent specs or prices.
- Return the JSON object only.`;
...
module.exports = { runAgent, runChat, runTranslate, runAgentTurn, runOrchestrator, runAppletAsk, runComposeApplet, runListModels, runAgentStep, runAgentPlan, runExtractSlides, runExtractDoc, runExtractSheets, runSynthesizeAgent, runBuildLesson, runBuildQuiz, runBuildCompare, runWatchCheck };
