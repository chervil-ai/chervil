...
// one, which makes any input safe and keeps the match conjunctive (all terms).
...
function createPageIndex(dbPath) {
  ...
  const api = {
     ...
     */
    search(text, { limit = 20, kind = null, includeBody = false } = {}) {
      const match = ftsQuery(text);
      if (!match) return [];
      const where = kind ? 'AND p.kind = ?' : '';
      const sql = `
        SELECT p.*, snippet(pages_fts, 2, '[', ']', '…', 12) AS snippet
    ...
    /** Privacy controls (RFC 0013): real deletes, not tombstones. */
    forgetSite(host) {
      if (!host) return;
