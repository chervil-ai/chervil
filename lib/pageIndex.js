...
function createPageIndex(dbPath) {
  ...
  const stmt = {
    ...
    // Trash weight, for the Library banner. length() on a TEXT column is the
    // character count, which is what the JSON blob costs anyway; html dominates
    // (a hero-bearing page is ~90% inlined base64 image) so the rest is noise.
    trashStats: db.prepare(`
      SELECT COUNT(*) AS n,
             COALESCE(SUM(LENGTH(COALESCE(html, '')) + LENGTH(COALESCE(body, ''))), 0) AS bytes,
             MIN(trashed_at) AS oldest
      FROM pages WHERE trashed_at IS NOT NULL
    `),
    // Age-based purge. Composed pages are never evicted while live — only once
    // the user has actually thrown them away, and only after the retention window.
    purgeTrashOlder: db.prepare('DELETE FROM pages WHERE trashed_at IS NOT NULL AND trashed_at < ?'),
  ...
  const api = {
    ...
    // `at` lets a caller preserve the ORIGINAL deletion time — absorbing a legacy
    // machine's trash must not restart the retention clock, or a page deleted two
    // months ago on another computer gets a fresh 30 days here.
    trash(id, at) { stmt.trash.run(Number(at) > 0 ? Number(at) : nowMs(), id); },
    restore(id) { stmt.restore.run(id); },
    remove(id) { stmt.remove.run(id); },
    // VACUUMs like the privacy controls below. Emptying the trash is the one place
    // a user explicitly says "this is gone" about their own pages; leaving the text
    // sitting in free pages afterwards (2.1MB of a 5.9MB file, measured) makes that
    // a lie and stops the file ever shrinking.
    emptyTrash() { stmt.emptyTrash.run(); db.exec('VACUUM'); },

    /**
     * True trash weight — count, bytes and the oldest item's age. `listTrash` takes
     * a LIMIT (the Library asks for 100), so counting its rows under-reports the
     * moment there are more; this reads the table directly and is what the Library
     * banner and the retention sweep both trust.
     */
    trashStats() {
      const r = stmt.trashStats.get() || {};
      return { count: r.n || 0, bytes: r.bytes || 0, oldestAt: r.oldest || 0 };
    },

    /**
     * Drop trashed pages older than `cutoffMs`. Returns how many went, so the
     * caller can decide whether the VACUUM is worth it — this runs on a timer, and
     * VACUUM rewrites the whole file, so it must not fire on a no-op sweep.
     */
    purgeTrashOlderThan(cutoffMs) {
      if (!Number.isFinite(cutoffMs) || cutoffMs <= 0) return 0;
      const before = stmt.countTrash.get().n;
      stmt.purgeTrashOlder.run(cutoffMs);
      const removed = before - stmt.countTrash.get().n;
